...
http.createServer((req, res) => {
  ...
  if (req.url.includes('/edcr/rest/dcr/scrutinize') || req.url.includes('/edcr/rest/dcr/scrutinydetails')) {
    let requestedEdcr = "PL-2024-1234";
    const url = new URL(req.url, `http://${req.headers.host}`);
    if (url.searchParams.get('edcrNumber')) {
      requestedEdcr = url.searchParams.get('edcrNumber');
    } else if (url.searchParams.get('transactionNumber')) {
      requestedEdcr = url.searchParams.get('transactionNumber');
    }
    const responseCopy = JSON.parse(JSON.stringify(scrutinyResponse));
    responseCopy.edcrDetail[0].edcrNumber = requestedEdcr;
    res.writeHead(200);
    res.end(JSON.stringify(responseCopy));
